    ...
    <main class="max-w-7xl mx-auto px-8 py-10">
        ...
        <div class="flex flex-col md:flex-row md:items-center justify-between mb-10 gap-4">
            ...
            <div class="flex items-center gap-3">
                <button
                    class="px-6 py-2.5 bg-white border border-slate-200 rounded-xl text-sm font-bold text-slate-600 hover:bg-slate-50 transition-all">
                    <i class="fas fa-plus mr-2 text-[10px]"></i> Request Matching
                </button>
                <a href="{{ route('student.search') }}"
                    class="px-6 py-2.5 bg-[#1a3652] text-white rounded-xl text-sm font-bold shadow-lg shadow-[#1a3652]/20 hover:bg-[#132a41] transition-all">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-10">
            ...
            @foreach($stats as $stat)
                <div class="bg-white p-6 rounded-[2rem] border border-slate-100 shadow-sm flex items-start justify-between">
                    <div>
                        <p class="text-slate-400 text-[11px] font-bold uppercase tracking-wider mb-3">{{ $stat['label'] }}
                        </p>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-12 gap-8">
            ...
            <div class="lg:col-span-8 space-y-8">
                ...
                <div class="bg-[#1a3652] rounded-[2.5rem] p-8 text-white relative overflow-hidden group">
                    <div
                        class="absolute top-[-20%] right-[-10%] w-64 h-64 bg-white/5 rounded-full blur-3xl group-hover:bg-white/10 transition-all">
                    </div>
                    <div class="relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-6">
                        <div class="flex items-center gap-6">
                            <div class="w-16 h-16 rounded-2xl overflow-hidden border-2 border-white/20 shadow-xl">
                                <img src="https://ui-avatars.com/api/?name=Andi+Firmansyah&background=random"
                                    alt="Tutor" class="w-full h-full object-cover">
                            </div>
                            <div>
                                <div
                                    class="flex items-center gap-2 text-[10px] font-bold uppercase tracking-widest text-emerald-400 mb-2">
                                    <span class="w-2 h-2 bg-emerald-400 rounded-full animate-pulse"></span> Sesi
                                    Berikutnya
                                </div>
                        ...
                        <button
                            class="bg-white text-[#1a3652] px-8 py-3 rounded-2xl font-bold text-sm hover:bg-slate-100 transition-all flex items-center gap-2">
            ...
            <div class="lg:col-span-4 space-y-6">
                    ...
                    <div class="space-y-6">
                        <div class="relative pl-4 border-l-2 border-emerald-500">
                            <div class="flex justify-between items-start mb-2">
                                <h4 class="text-xs font-bold leading-relaxed pr-8">Python Pandas untuk Data Analysis
                                </h4>
                                <span
                                    class="bg-emerald-500 text-white text-[10px] font-bold px-2 py-1 rounded-lg">88</span>
                            </div>
                            <p class="text-[10px] text-slate-400 leading-relaxed mb-1">Pemahaman konsep DataFrame sangat
                                baik. Perlu latihan lebih pada operasi merge dan groupby kompleks.</p>
                        ...
                        <div class="relative pl-4 border-l-2 border-yellow-500">
                            <div class="flex justify-between items-start mb-2">
                                ...
                                <span
                                    class="bg-yellow-500 text-white text-[10px] font-bold px-2 py-1 rounded-lg">76</span>
                            </div>
                            <p class="text-[10px] text-slate-400 leading-relaxed mb-1">Konsep limit sudah paham, namun
                                perlu memperkuat pemahaman chain rule pada fungsi komposit.</p>
                            <p class="text-[9px] font-bold text-slate-300 uppercase">8/4/2026</p>
                        </div>
                    </div>
                    <button
                        class="w-full mt-8 py-3 bg-white border border-dashed border-slate-200 rounded-xl text-[10px] font-black uppercase tracking-widest text-slate-400 hover:border-slate-400 hover:text-slate-600 transition-all">
                ...
                <div class="bg-slate-50 rounded-[2.5rem] p-8 border border-slate-100/50">
                    ...
                    <div class="space-y-3">
                        <button
                            class="w-full py-3.5 bg-[#1a3652] text-white rounded-xl text-xs font-bold shadow-lg shadow-[#1a3652]/10 hover:-translate-y-0.5 transition-all">Request
                            Matching</button>
                        <button
                            class="w-full py-3.5 bg-white border border-slate-200 text-slate-600 rounded-xl text-xs font-bold hover:bg-white transition-all">Cari
                            Tutor Baru</button>
                        <button
                            class="w-full py-3.5 text-slate-400 text-xs font-bold hover:text-[#1a3652] transition-colors">Lihat
                            Monitoring Board</button>
                    </div>
                </div>
            </div>
        </div>
